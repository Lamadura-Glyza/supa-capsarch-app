...
export const deleteAuthUser = async (userId) => {
  try {
    if (!supabaseAdmin?.auth?.admin?.deleteUser) {
      throw new Error('Admin deleteUser not available');
    }
...
export const promoteTeacherToAdmin = async (userId) => {
  try {
    const { data, error } = await supabaseAdmin
      .from('user_profiles')
      .update({ role: 'teacher_admin' })
      .eq('user_id', userId)
      .eq('role', 'teacher');
    ...
    console.error('promoteTeacherToAdmin error:', error);
    throw error;
  }
};

export const demoteTeacherAdmin = async (userId) => {
  try {
    const { data, error } = await supabaseAdmin
      .from('user_profiles')
      .update({ role: 'teacher' })
    ...
    if (error) {
      console.error('Error demoting teacher admin:', error);
      throw error;
    }

    return data;
  } catch (error) {
    console.error('demoteTeacherAdmin error:', error);
    throw error;
  }
}; 